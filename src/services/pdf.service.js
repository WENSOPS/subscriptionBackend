...
function pdfCurrency(amount, currency) {
  ...
  for (const [sym, alt] of Object.entries(PDF_CURRENCY_SYMBOLS))
    str = str.replaceAll(sym, alt);
  return str;
}
...
export async function fetchImageBuffer(url, _hops = 0) {
  ...
  return new Promise((resolve, reject) => {
    ...
    req.on("timeout", () => {
      req.destroy();
      reject(new Error("Image fetch timed out after 10s"));
    });
    req.on("error", reject);
  });
}
...
async function generateInvoicePDF(invoice) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const HDR_TOP = 22;
    if (logoBuffer) {
      doc.image(logoBuffer, L, HDR_TOP, { fit: [60, 60] });
    }
    ...
    doc
      ...
      .font("Helvetica-Bold")
      .fillColor(INK)
      .text(company.name, cX, HDR_TOP);
    let chy = HDR_TOP + 28; // detail lines start at y=50, matching side margins
    doc.fontSize(8.5).font("Helvetica").fillColor(SUBINK);
    if (company.address) {
      doc.text(company.address, cX, chy, { width: 250 });
      chy += 12;
    }
    if (company.mobile) {
      doc.text("Tel: " + company.mobile, cX, chy);
      chy += 12;
    }
    if (company.email) {
      doc.text(company.email, cX, chy);
      chy += 12;
    }
    if (company.website) {
      doc.text(company.website, cX, chy);
      chy += 12;
    }
    doc
      .fontSize(32)
      .font("Helvetica-Bold")
      .fillColor(ACCENT)
      .text("INVOICE", L, HDR_TOP, { width: W, align: "right" });
    ...
    doc
      ...
      .font("Helvetica-Bold")
      .fillColor(MUTED)
      .text("BILL TO", L, BILL_Y);
    doc
      .fontSize(12)
      .font("Helvetica-Bold")
      .fillColor(INK)
      .text(customer.name, L, BILL_Y + 12);
    let clY = BILL_Y + 28;
    doc.fontSize(9).font("Helvetica").fillColor(SUBINK);
    if (customer.mobile) {
      doc.text("Tel: " + customer.mobile, L, clY);
      clY += 13;
    }
    if (customer.email) {
      doc.text(customer.email, L, clY);
      clY += 13;
    }
    ...
    doc
      ...
      .stroke();
    doc
      .moveTo(L, TABLE_Y + ROW_H)
      .lineTo(R, TABLE_Y + ROW_H)
      .strokeColor(BORDER)
      .lineWidth(0.5)
      .stroke();
    doc
      ...
      .font("Helvetica-Bold")
      .fillColor(INK)
      .text("Package Name", C0x + 8, TABLE_Y + 9, {
        width: C0w - 14,
        align: "left",
      })
    ...
    for (let i = 0; i < lineItems.length; i++) {
      ...
      doc
        ...
        .text(pdfCurrency(item.unitPrice, item.currency), C2x + 2, ry + 8, {
          width: C2w - 6,
          align: "right",
        })
        .text(pdfCurrency(lineTotal, item.currency), L, ry + 8, {
          width: W - 10,
          align: "right",
        });
      ry += ROW_H;
    ...
    if (stampBuffer) {
      const sz = 72;
      doc.image(stampBuffer, SIG_X + (SIG_W - sz) / 2, FOOT_Y, {
        fit: [sz, sz],
      });
    }
    const sigLineY = FOOT_Y + (stampBuffer ? 80 : 22);
    doc
      .moveTo(SIG_X, sigLineY)
      .lineTo(R, sigLineY)
      .strokeColor(INK)
      .lineWidth(0.4)
      .stroke();
    doc
      ...
      .font("Helvetica-Bold")
      .fillColor(INK)
      .text("Authorized Signature", SIG_X, sigLineY + 7, {
        width: SIG_W,
        align: "center",
      });
    doc
      ...
      .font("Helvetica")
      .fillColor(MUTED)
      .text("for (" + company.name + ")", SIG_X, sigLineY + 19, {
        width: SIG_W,
        align: "center",
      });
    doc.end();
