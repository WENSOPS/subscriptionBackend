...
export const createBookingValidation = [
  ...
  body('purchaseAmount').isFloat({ gt: 0 }).withMessage('Purchase amount must be a positive number'),
  validate,
];

export const updateStatusValidation = [
  body('status').isIn(['initiated', 'pending', 'active', 'cancelled', 'completed', 'failed']).withMessage('Status must be one of: initiated, pending, active, cancelled, completed, failed'),
  validate,
];
